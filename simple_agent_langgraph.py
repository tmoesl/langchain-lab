...
# TODO: Remove this node and use prebuilt ToolNode(tools) instead of custom implementation
def tool_node(state: AgentState) -> dict:
    """Performs the tool calls and returns the response"""
    last_message = state["messages"][-1]  # instance of AIMessage
    response = []
...
# =============================
# Simple approach - everything handled for you
...
message = [HumanMessage(content="Multiply 2 by 5 and then add 10.")]
response = agent.invoke({"messages": message})  # type: ignore
print(f"Assistant: {response['messages'][-1].content}")
